...
### Тормоза, зависание и вылет: приложение брало полный кадр

Три разные с виду жалобы оказались одной причиной. Аппаратного
обработчика кадра у этого чипа в mainline нет, цвет из матрицы Байера
собирает центральный процессор. А приложение просмотра берёт
наибольший из предложенных размеров — полный кадр матрицы: 3264x2448
по четыре байта на точку, 32 МБ на кадр.

Что процессор реально тянет (задняя камера):

| размер кадра | кадров в секунду |
|---|---|
| 3264x2448 | 5,6 |
| 1632x1224 | 21,2 |
| 1280x960 | 27,8 |
...
Пять с половиной кадров в секунду — это и есть «тормозит». А поток
32-мегабайтных кадров при гигабайте памяти вычерпывал её всю: в
журнале появлялось `VFE0 rdi0 overflow`, затем система вставала
намертво и уходила в сброс по сторожевому таймеру.

Полезный признак при разборе таких зависаний: `pstore` пуст и записи
об ошибке в журнале нет, журнал просто обрывается. Это отличает
жёсткий зависон от паники ядра — искать ошибку ядра там незачем.

Лечится ограничением в `simple` — обработчике конвейера libcamera:
когда цвет собирается программно, предлагаемые размеры обрезаются до
1632x1224. Необработанный поток не трогается, полный кадр остаётся
доступен тому, кто попросит его прямо.
