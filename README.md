...
Остались предупреждения, на обнаружение не влияющие: libcamera не знает
эти датчики в своей базе, поэтому не определяет, какая камера передняя,
а какая задняя, и не имеет данных для точной автоэкспозиции.
...
В стоковой библиотеке `libmmcamera_s5k4h5yc.so` нашлись три таблицы
режимов, среди них — уменьшенный `816x460` с прореживанием 1 к 4.
Полезная подробность: ФАПЧ у всех режимов одинакова, различаются только
длина строки и число строк, так что частоты менять не пришлось.
...
Патч — `kernel/0015-s5k4h5-preview-mode.patch`; полный кадр остался для
съёмки, уменьшенный выбирается по запрошенному размеру.
...
### Ещё одно ограничение: обе камеры делят приёмник

Переключение между камерами подвешивает приложение:

```
Failed to setup link 'msm_csiphy1'[1] -> 'msm_csid0'[0]: Resource busy
```

Обе садятся на `csid0`, и вторая не запускается, пока работает первая.
По отдельности обе работают. У Samsung в стоке приёмники разведены:
задняя на `csid0`, фронтальная на `csid1` (`qcom,csid-sd-index`).
Развести их — следующий шаг; выбор делает libcamera, обходя граф
связей, и её нужно к этому склонить.
